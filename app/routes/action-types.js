let {
    ...
    deleteActionType,
    editActionType
} = require('../controllers/ActionTypes')
...
module.exports = (router) => {
    ...
    router.get('/:id', async (req, res) => {
        try {
            var district = await getActionType(req.params.id, req.query)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
    ...
    router.post('/', async (req, res) => {
        try {
            var district = await createActionType(req.body)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
    ...
    router.patch('/:id', async (req, res) => {
        try {
            var district = await editActionType(req.params.id, req.body)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
    ...
    router.delete('/:id', async (req, res) => {
        try {
            var district = await deleteActionType(req.params.id)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
