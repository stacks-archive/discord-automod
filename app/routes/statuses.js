...
module.exports = (router) => {
    ...
    router.get('/', async (req, res) => {
        ...
        try {
            var districts = await getStatuses(
                req.query,
                req.query.page,
                req.query.perPage,
            )
    ...
    router.get('/:id', async (req, res) => {
        try {
            var district = await getStatus(req.params.id, req.query)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
    ...
    router.post('/', async (req, res) => {
        try {
            var district = await createStatus(req.body)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
    ...
    router.patch('/:id', async (req, res) => {
        try {
            var district = await editStatus(req.params.id, req.body)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
    ...
    router.delete('/:id', async (req, res) => {
        try {
            var district = await deleteStatus(req.params.id)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
