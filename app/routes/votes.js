...
module.exports = (router) => {
    ...
    router.get('/:id', async (req, res) => {
        try {
            var district = await getVote(req.params.id, req.query)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
    ...
    router.post('/', async (req, res) => {
        try {
            var district = await createVote(req.body)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
    ...
    router.patch('/:id', async (req, res) => {
        try {
            var district = await editVote(req.params.id, req.body)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
    ...
    router.delete('/:id', async (req, res) => {
        try {
            var district = await deleteVote(req.params.id)
        } catch (error) {
            if (error.statusCode === 500) {
                console.error(error)
            }
